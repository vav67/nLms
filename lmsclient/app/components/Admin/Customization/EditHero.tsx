...
const EditHero: FC<Props> = (props: Props) => {
  ...
  const { data, refetch } = useGetHeroDataQuery("Banner", {
    refetchOnMountOrArgChange: true,  //иногда нужно обновлять данные
  });
   //запрос редактирования макета мутированием
  const [editLayout, { isLoading, isSuccess, error }] = useEditLayoutMutation();

   useEffect(() => {
    if (data) { //результат если запрошен баннер
      setTitle(data?.layout?.banner.title);
      setSubTitle(data?.layout?.banner.subTitle);
      setImage(data?.layout?.banner?.image?.url);
    }
     //результат запроса редактирования макета
    if (isSuccess) {
      toast.success("Hero updated successfully!");
      //пока ненужно refetch(); //обновим запрос
    }
    if (error) {
      if ("data" in error) {
  ...
  // редактируем  изменения макет это тип баннер имаге титтл и субтитл
  const handleEdit = async () => {
    await editLayout({  type: "Banner",  image,
                        title,   subTitle, });
  };
    ...
    <>
      <div className="w-full 1000px:flex items-center">
        <div className="absolute top-[100px] 1000px:top-[unset] 1500px:h-[700px] 1500px:w-[700px] 
                 1100px:h-[500px] 1100px:w-[500px] h-[50vh] w-[50vh] hero_animation rounded-[50%]
                  1100px:left-[18rem] 1500px:left-[21rem]"></div>
        <div className="1000px:w-[40%] flex 1000px:min-h-screen items-center justify-end pt-[70px] 
                        1000px:pt-[0] z-10">
